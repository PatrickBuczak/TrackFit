...
//Methode zur Instanzierung von Abhängigkeiten wie im Backend durch Autowired
  ...
  loadFriends(): void {//Freunde des Users werden geladen und in friends eingefügt (array)
    this.http.get<string[]>(`http://localhost:8080/api/v1/friends/list?username=${this.logedusername}`).subscribe({
      next: (friendsList) => {
  ...
  mapActivityType(activityType: string): string {
    ...
    return mapping[activityType] || activityType; // Fallback: Originalwert zurückgeben
  }
...
//Für jede Aktivität wird die Methode this.getProfilePictureForUser(activity.username) ausgeführt indem
        ...
        this.data = response //danach werden alle Aktivitäten in data geladen aber zuvor gemaped
          .map((activity) => { //jede Aktivität aus Repsonse wird einzeln wie folgt bearbeitet
            ...
            const isFriend = this.friends.includes(activity.username);
            //Checkt ob in friends der Username der Aktivität enthalten ist
            //wenn ja dann true, sonst false
            //der eigene Name kann ja nicht enthalten sein
            ...
            if (activity.visibility === 'nur Freunde' && !isOwner && !isFriend) {
              return null;
            }
            //wenn die Aktivität nur von freunden eingesehen werden kann
            //der User aber nicht der besitzer oder der besitzer der Aktivität auch nicht in der
            // Freundesliste ist
            //dann kann er diese ja auch nicht einsehen logischerweise
            //deswegen return
            //sonst
            ...
            return {//dadurch wir ein neues Objekt erstellt
              ...
              likeCount: activity.likes ? activity.likes.length : 0,
              ...
              isFriend, //gleiches
              userHasLiked,
              marker: this.getMarker(activity, isOwner, isFriend), //ruft die getMarker auf
              //die Eigenschaft marker wird hinzugefügt und durch den wert der Methode betsimmt
              //eigene Aktivität? nur für Freunde?
              showDetails: false,
              //ist eine neue Eigenschaft die auf false gesetzt wird
          ...
          .filter((item) => item !== null) //Berücksichtigt nur die
          //aktivitäten die für den User sichtbar sind und so durch filtereddate gekennzeichnet wurden
      ...
      if (!this.userProfilePictures[username]) {//guckt ob der Username bereits in userProfilePictures VariableXArray drin ist
        ...
        this.http.get<any>(`http://localhost:8080/api/v1/user/get-user?username=${username}`).subscribe({
          next: (user) => {
            ...
            this.userProfilePictures[username] = user.profilepicture || 'assets/profilicon.png'; // Standardbild, falls kein Profilbild vorhanden
  ...
  getMarker(activity: any, isOwner: boolean, isFriend?: boolean): string {
    //die Konstante isOwner des Mappings und die Aktivität als Array werden übergeben
    if (activity.visibility === 'nicht sichtbar' && isOwner) {
      return 'yellow';
    } else if (activity.visibility === 'nur Freunde' && (isFriend || isOwner)) {
      return 'green';
    }
    return 'blue';
  }
  ...
  toggleActivityDetails(activity: any): void {
    ...
    console.log(`showDetails für "${activity.activityname}":`, activity.showDetails);
  }


  //kenan
  toggleLikes(activity: any): void {
    // Verhindere das Liken der eigenen Aktivität
    if (activity.username === this.logedusername) {
      console.log("Du kannst deine eigene Aktivität nicht liken!");
      return;
    }
  ...
  isActivityVisible(activity: any): boolean {
    if (activity.visibility === 'nicht sichtbar' && activity.username !== this.logedusername) {
      //ist nicht sichtbar und nicht eigene = false
      return false;
    }
  ...
  filteredData(): any[] {
    //alle Elemente von Data werden gefiltert
    //jede betrachetete Aktivität (item)
    return this.data.filter(item =>
      this.isActivityVisible(item) && //übergibt halt die Aktivität der methode oben
      item.activityname.toLowerCase().includes(this.searchText.toLowerCase())
      //alle Elemente die Sichtbar sind oder einem selbst gehöre werden angezeigt
      //die Freunde wurden ja schon in getAllActivities() gefiltert
    );
  }

  //kenan
  updateActivityLikes(activity: any): void {
